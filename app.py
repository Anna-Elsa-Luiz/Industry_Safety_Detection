...
def download_model_s3() -> str:
    ...
    try:
        model_download_dir = os.path.join("yolov7", model_pusher_config.S3_MODEL_KEY_PATH)
...
def predictRoute():
    try:
        ...
        logging.info("Successfully downloaded model file")

        os.system(f"cd yolov7 && python detect.py --weights {model_file} --source ../{clApp.filename}")
        ...
        logging.error(f"Value error: {val}")
